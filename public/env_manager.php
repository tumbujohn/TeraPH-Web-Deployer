...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $project) {
    ...
    if ($postAction === 'save_template') {
        ...
        flash('success', 'Template saved. ' . count($keys) . ' key(s) synced.');
        header('Location: env_manager.php?project_id=' . $projectId);
        exit;
    }
    ...
    if ($postAction === 'import_example') {
        ...
        if (!file_exists($examplePath)) {
            ...
            EnvVar::syncKeys($projectId, $keys);
            flash('success', 'Imported .env.example from target. ' . count($keys) . ' key(s) synced.');
        }
        header('Location: env_manager.php?project_id=' . $projectId);
        exit;
    }
    ...
    if ($postAction === 'save_values') {
        ...
        foreach ($existing as $i => $var) {
            ...
            // If blank was submitted and an existing value is set, preserve it
            // (user didn't clear it deliberately; empty = "don't change")
    ...
    if ($postAction === 'add_key') {
        $newKey = strtoupper(trim($_POST['new_key'] ?? ''));
        if (!preg_match('/^[A-Z0-9_]+$/', $newKey)) {
            flash('error', 'Key name may only contain uppercase letters, digits, and underscores.');
        } else {
            ...
            if (in_array($newKey, $existing, true)) {
                ...
                // Append key to template if one exists
                if (!empty($project['env_template'])) {
                    $template  = rtrim($project['env_template']) . "\n" . $newKey . "=\n";
                    $pdo       = Database::connect();
                    $pdo->prepare("UPDATE projects SET env_template = ?, updated_at = ? WHERE id = ?")
                        ->execute([$template, date('Y-m-d H:i:s'), $projectId]);
                }
...
<head>
    ...
    <style>
        ...
        .template-area     { width: 100%; min-height: 220px; font-family: monospace; font-size: 12px;
                             ...
                             padding: 12px; color: #e5e7eb; resize: vertical; outline: none; }
        ...
        .add-key-input     { flex: 1; font-family: monospace; font-size: 12px; background: #0a0a0a;
                             ...
                             color: #e5e7eb; outline: none; text-transform: uppercase; }
...
<main class="main-content">
    <div class="container">
        ...
        <div class="env-grid">
            ...
            ================================================================ -->
            <div class="env-card">
                <h3>Template <span style="color:#555; font-weight:400;">(.env.example structure)</span></h3>
                <p class="section-hint">
                    Paste your <code>.env.example</code> here. Only <code>KEY=</code> lines are
                    used — values in the template are replaced by the stored values below at deploy
                    time. Comments and blank lines are preserved in the rendered output.
                </p>
                <form method="post">
                    <?= $csrfField ?>
                    ...
                    <div class="btn-row">
                        ...
                        <form method="post" style="display:inline;">
                            ...
                            <button type="submit" class="btn btn-ghost btn-sm"
                                    title="Read .env.example from target directory">
                                ↓ Import from .env.example
                            </button>
                        </form>
                        <?php else: ?>
                        <span style="font-size:11px; color:#555; align-self:center;">
                            (No .env.example found in target)
                        </span>
                        <?php endif; ?>
                    </div>
                </form>
            ...
            <div class="env-card env-full">
                    ...
                    <table class="key-table">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>
                                    <div class="val-wrap">
                                        <input
                                            ...
                                            placeholder="<?= $var['value'] !== '' ? '(value set — leave blank to keep)' : 'empty' ?>"
                                            id="val-<?= h($var['key_name']) ?>"
                                        >
                                        <button type="button" class="btn-eye"
                                                onclick="toggleVal('<?= h($var['key_name']) ?>', this)">Show</button>
                                    </div>
                                </td>
                                ...
                                <td>
                                    <form method="post" style="display:inline;"
                                          onsubmit="return confirm('Delete key <?= h(addslashes($var['key_name'])) ?>?')">
                                        <?= $csrfField ?>
                                        <input type="hidden" name="form_action" value="delete_key">
                                        <input type="hidden" name="del_key" value="<?= h($var['key_name']) ?>">
                                        <button type="submit" class="btn-del" title="Delete key">✕</button>
                                    </form>
                                </td>
                            </tr>
                            <?php endforeach; ?>
                ...
                <form method="post" class="add-key-row">
                    ...
                    <input type="text" name="new_key" class="add-key-input"
                           placeholder="NEW_KEY_NAME" pattern="[A-Za-z0-9_]+"
